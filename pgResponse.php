...
if($isValidChecksum == "TRUE") {
    ...
    $oArr = explode('_', $oid);
    $oid = isset($oArr[0]) ? $oArr[0] : ''; // Ensure $oid is set
    $TXNID = isset($_POST['TXNID']) ? $_POST['TXNID'] : ''; // Check if TXNID exists

    if (!empty($oid) && !empty($TXNID)) {
        // Corrected SQL query with table name
        mysqli_query($con, "UPDATE order_master SET payment_status='w', payment_id='$TXNID' WHERE id='$oid'");
    } else {
        // Log the error for debugging
        error_log("Missing ORDERID or TXNID in POST data");
    }
    redirect(FRONT_SITE_PATH . 'error');
}
